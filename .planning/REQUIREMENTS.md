...
- [ ] **INFRA-01**: Extension maintains analysis session across service worker dormancy events without data loss (long-lived port or `storage.session` flush)
...
- [ ] **SCOPE-01**: User can click to select a specific element or component on the page, and the extension captures only that subtree's HTML, computed styles, and assets — scoped output rather than full-page dump
...
- [ ] **SCOPE-03**: Extension fetches and saves actual image, font, and icon files (routed through the background service worker to bypass CORS) rather than capturing URL references only
...
- [ ] **TRACK-02**: Extension annotates the DOM tree with logical component boundaries (React fiber internals, `data-` attributes, BEM class patterns) to produce a named component hierarchy alongside the raw HTML
