...
| TRACK-03 | All detection logic is expressed as generic observable patterns with no hardcoded site names in the codebase | Audit confirms 2 comment references (content.js:337, popup.js:484) + site4source.html + output.json with Site2 URLs — all must be removed or genericized |
</phase_requirements>
...
├── test-page.html         # Renamed from site4source.html (generic test fixture)
└── [site4source.html]    # DELETE or rename
```
...
```javascript
// content.js — replace "like what we see in site4" comment + pattern
...
- `output.json` in repo root: Contains Site2 URLs, violates TRACK-03. Delete or add to `.gitignore`.
...
3. **output.json disposition**
   - What we know: Contains Site2 page content — violates TRACK-03.
...
| TRACK-03 | No site names in codebase | Automated grep | `grep -ri "site4\|site1\|site2" . --include="*.js" --include="*.html"` |

### Sampling Rate
- Per task commit: Run the grep audit: `grep -ri "site4\|site1\|site2" . --include="*.js" --include="*.html" --include="*.json" | grep -v ".planning" | grep -v "node_modules"`
